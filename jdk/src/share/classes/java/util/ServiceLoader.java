...
public final class ServiceLoader<S>
    ...
    private class LazyIterator
        ...
        private boolean hasNextService() {
            // 缓存着下一个 name。若有，则立即返回
            if (nextName != null) {
                return true;
            }
            if (configs == null) {
                try {
                    // 前缀就是 META-INF/services/
                    // 拼接上 service 完全限定名
            ...
            // pending == null || !pending.hasNext 意味着 URL 没有读取到有意义的东西
            // 那就继续从 configs 读取
            while ((pending == null) || !pending.hasNext()) {
                if (!configs.hasMoreElements()) {
                    return false;
                }
                // 解析出来一个 pending，这是一个迭代器，用于遍历 URL 指示的文件中的每一行
    ...
    public Iterator<S> iterator() {
        return new Iterator<S>() {
            ...
            // 之前可能因为多次调用 next 产生了许多实例缓存。他们位于 providers。
            // 通过 providers 创建迭代器（第一次肯定什么也没有）
            Iterator<Map.Entry<String, S>> knownProviders
                    = providers.entrySet().iterator();

            public boolean hasNext() {
                // 如果第一次 providers 没有，那么这里一定是 false
                if (knownProviders.hasNext())
                    return true;

                // 那么，就使用 lookupIterator 探查下一个 name（接口的名字）
     ...
     */
    public static <S> ServiceLoader<S> load(Class<S> service) {
        // 给定 Service 的 Class 对象，从类路径加载

        // 获取线程上下文的 ClassLoader
