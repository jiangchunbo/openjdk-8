...
public class Launcher {
    ...
    static class ExtClassLoader extends URLClassLoader {
        ...
        private static File[] getExtDirs() {
            // 获取 ext dir 属性字符串，使用 File.pathSeparator 分割
            String s = System.getProperty("java.ext.dirs");
            File[] dirs;
            if (s != null) {
                // 使用 StringTokenizer 对字符串进行分割处理
    ...
    private static File[] getClassPath(String cp) {
        ...
        if (cp != null) { // 如果 cp != null，就算没有分隔符 : 起码也有一个元素吧
            int count = 0; // 下面 File[] 的索引
            int maxCount = 1; // 默认 1 个
            int pos = 0, lastPos = 0;

            // Count the number of separators first
            // 计算有多少个分隔符
            // 例如 A:B:C，有一个分隔符就将 maxCount++ => 这样就可以得到数组大小
            while ((pos = cp.indexOf(File.pathSeparator, lastPos)) != -1) {
                maxCount++;
                lastPos = pos + 1; // 为下次循环做准备
            }
            ...
            // 裁剪 array 到正确的大小，但是什么时候大小计算有错？我暂时没想到
            if (count != maxCount) {
