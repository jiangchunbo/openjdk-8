...
bool enqueue_discovered_ref_helper(ReferenceProcessor* ref,
  ...
  // 将那些没有再次变得 active 的 references 入队
  // 且为下一轮收集清理现场
...
void ReferenceProcessor::enqueue_discovered_reflist(DiscoveredList& refs_list,
  ...
  // 初始化遍历，由于之前发现阶段已经维护好了 discovered ，这是一个指针
  // 所以，只需要不断使用 discovered 移动到下一个，直到 obj == next_d
  oop obj = NULL; // 当前 obj 需要处理的引用对象
  oop next_d = refs_list.head(); // 下一个元素
  if (pending_list_uses_discovered_field()) { // New behaviour
    ...
    while (obj != next_d) {
      ...
      if (next_d == obj) {  // obj is last
        ...
        // 如果原来没有，就 obj 指向自己（老套路，自己是最后一个，自己指向自己）
        if (old == NULL) {
...
void ReferenceProcessor::enqueue_discovered_reflists(HeapWord* pending_list_addr,
  ...
  if (_processing_is_mt && task_executor != NULL) {
    ...
    // _max_num_q 可能是 1 吧
    // number_of_subclasses_of_ref 子类的数量，值是 4 ,软弱虚终
    for (uint i = 0; i < _max_num_q * number_of_subclasses_of_ref(); i++) {
...
ReferenceProcessor::process_phase3(DiscoveredList&    refs_list,
  ...
  while (iter.has_next()) {
    ...
    // 是否要 clear referent，其实就是赋值 null
    if (clear_referent) {
  ...
  AbstractRefProcTaskExecutor* task_executor)
{
  ...
  // 若 discovery 阶段是并行的，或者当前处理也打算并行，且 ParallelRefProcBalancingEnabled == true，则 balance_queues 把个线程链表重新分配，使每条链表长度更平均
  // 目的：避免处理阶段某些线程链表太短造成 CPU 空闲，提升并行度
  ...
  //   such referents.
  // 仅对 Soft 链表执行。怎么保证的呢？policy 这个参数只有 Soft 处理的时候才会传，其他都是 NULL
  if (policy != NULL) {
    if (mt_processing) {
  ...
  // 如果对象是存活的，就要从 list 中删除
  // 无论是哪种引用，只要 referent 在标记阶段被证明是 alive 的，就把此节点从链表中删除（因为它不需要进一步处理，GC 之后仍然可达）
  // 可能当初是因为标记中途，加入进来了，也可能是并发标记的问题加入进来了
  //
  if (mt_processing) {
  ...
  // 阶段 3:真正处理引用
  // 针对仍然在链表中的每个引用（即：referent 已死，且 SoftReference 也没有因为策略被保活）
  // - 若 clear_referent == true，则 referent 置为 NULL，帮助后续内存回收
  // - 执行虚引用/final引用的 enqueue、finalize() 或加入 ReferenceQueue 等 VM 特定动作
  if (mt_processing) {
...
bool ReferenceProcessor::discover_reference(oop obj, ReferenceType rt) {
  ...
  // 若引用对象的 next 不是 NULL，就不处理
  // next 不是 null 说明已经至少是即将进入 pending 状态了
  ...
  // 引用对象 obj 的字段 discovered 地址，有什么用，可以设置下一个引用对象
  ...
  // 根据 rt 得到当前线程的 list
  // 如果是单线程 discover，就是从数组里面拿第 0 个
  DiscoveredList* list = get_discovered_list(rt);
  if (list == NULL) {
    return false;   // nothing special needs to be done
  }

  // _discovery_is_mt 是否多线程发现
  if (_discovery_is_mt) {
    ...
    // The last ref must have its discovered field pointing to itself.
    // 头插法，所以 next 就是当前 obj 的下一个
    // 这里要多想想，如果是第一个元素插入进来，这个元素的 discovered 指向自己，这个特殊之处可以判定遍历的时候是否到头
    oop next_discovered = (current_head != NULL) ? current_head : obj;
    ...
    // 等价于 obj.discovered = next_discovered 。 赋值而已
