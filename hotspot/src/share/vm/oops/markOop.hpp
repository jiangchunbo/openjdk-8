...
public:
    ...
    enum
    {
        ...
        // 掩码，末尾 3 bit，翻译过来叫偏向锁掩码？
        biased_lock_mask = right_n_bits(lock_bits + biased_lock_bits), // (2 + 1)
        // lock_shift 是 0，其实不需要偏移
    ...
    JavaThread* biased_locker() const
    {
        ...
        return (JavaThread*)((intptr_t)(mask_bits(
            value(), ~(biased_lock_mask_in_place | age_mask_in_place | epoch_mask_in_place))));
    }
    ...
    /**
     * 是否是中立的。低位是 0b001 时，表示不偏向，也不被任何线程持有。
     */
    bool is_neutral() const
    {
        // value() 取出对象 mark word 的原始无符号数值
        // biased_lock_mask_in_place 这是一个 3 bit 111 的掩码，如果锁的掩码 == unlocked_value(1)，其实就是 0b001 其实就是无锁
        return (mask_bits(value(), biased_lock_mask_in_place) == unlocked_value);
    }
