...
{
    // 是否使用偏向锁(JVM 需要打开偏向锁的功能，似乎是默认开启？）
    if (UseBiasedLocking)
    {
        // 安全点是 JVM 规定的一个时刻或者说位置：当且仅当所有 Java 线程都暂停在这种位置时，
        // JVM 可以安全地做一些全局操作（如 STW GC、类卸载、偏向锁批量撤销、栈重写等）
        // 进入安全点后，线程堆栈处于已知形态，寄存器和解析器状态也都是可遍历的，从而保证 JVM 能够正确地扫描和修改对象状态
...
void ObjectSynchronizer::slow_enter(Handle obj, BasicLock* lock, TRAPS)
{
    ...
    if (mark->is_neutral())
    {
        ...
        // 将 mark 存储到 displaced header
        lock->set_displaced_header(mark);

        // 使用 CAS 把 mark 改为 lock 的地址指针
    ...
    else if (mark->has_locker() && THREAD->is_lock_owned((address)mark->locker()))
    {
        ...
        // lock record dispalced header 置为 null、
        // 可以用于区分首次加锁，还是重入
  ...
  if (mark->has_monitor() && mark->monitor()->is_entered(THREAD)) {
    ...
    // 如果走到这一步，意味着上面的获取锁都失败了，那么就将 displaced 存储为 3
...
(JavaThread* self, Handle h_obj)
{
    ...
    if (mark->has_monitor())
    {
        ...
        return (owner == self ||
                   self->is_lock_owned((address)owner))
                   ? owner_self
                   : owner_other;
    }
